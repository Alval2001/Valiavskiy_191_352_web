...
def load_user(user_id):
    ...
    search_user = "SELECT * FROM users2 WHERE id = %s;"
...
def load_roles():
    cursor = mysql.connection.cursor(named_tuple=True)
    search_user = "SELECT id, name FROM roles2;"
...
def login():
    if request.method == 'POST':
        ...
        if user_data['login'] and user_data['password']:
            cursor = mysql.connection.cursor(named_tuple=True)
            search_user = "SELECT * FROM users2 WHERE login = %s AND password_hash = SHA2(%s, 256);"
            cursor.execute(search_user, (
...
def users():
    cursor = mysql.connection.cursor(named_tuple=True)
    search_user = "SELECT users2.*, roles2.name AS role_name FROM users2 LEFT OUTER JOIN roles2 ON users2.role_id = roles2.id;"
    cursor.execute(search_user)
...
def show(user_id):
    ...
    search_user_role = "SELECT * FROM roles2 WHERE id = %s;"
...
def delete(user_id):
    with mysql.connection.cursor(named_tuple=True) as cursor:
        try:
            search_user = "DELETE FROM users2 WHERE id = %s;"
            cursor.execute(search_user, (user_id,))
        except connector.errors.DatabaseError as err:

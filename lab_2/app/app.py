    ...
    if request.method == 'POST':
        ...
        if not re.search(r'[^\d\s\(\)\-\.\+]', tel):
            ...
            if len(d_tel) == 10:
                ...
                tel_error = "Недопустимый ввод. Неверное количество цифр.» – если в номере не 10 или 11 цифр (номер должен содержать 11 цифр если он начинается с «+7» или «8», в остальных случаях – 10 цифр)"
                return render_template('telephone.html', tel=tel, tel_error=tel_error)
            tel = ''.join(d_tel)
            tel = f"8-{tel[1:4]}-{tel[4:7]}-{tel[7:9]}-{tel[9:]}"
        else:
            tel_error = "Недопустимый ввод. В номере телефона встречаются недопустимые символы"
            return render_template('telephone.html', tel=tel, tel_error=tel_error)
        return render_template('telephone.html', tel=tel)
